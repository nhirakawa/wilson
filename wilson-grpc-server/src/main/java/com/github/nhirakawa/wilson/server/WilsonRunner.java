package com.github.nhirakawa.wilson.server;
...
public class WilsonRunner {
  ...
  public static void main(String... args) throws IOException {
    ...
    boolean isLocalCluster = config.getBoolean(
  ...
  private static void runLocalCluster(Config config) throws IOException {
    Set<ClusterMember> clusterMembers = ObjectMapperWrapper
      .instance()
      .readValue(
        config
          .getList(ConfigPath.WILSON_CLUSTER_ADDRESSES.getPath())
    ...
    for (ClusterMember clusterMember : clusterMembers) {
      executorService.execute(
        () -> runMember(clusterMember, clusterMembers, config)
      );
    }
  }

  private static void runLocalMember(Config config) throws IOException {
    ClusterMember localClusterMember = ObjectMapperWrapper
      .instance()
      .readValue(
        config
          .getObject(ConfigPath.WILSON_LOCAL_ADDRESS.getPath())
          .render(ConfigRenderOptions.concise()),
  ...
  private static void runMember(
    ClusterMember clusterMember,
    Set<ClusterMember> clusterMembers,
    Config config
  ) {
    Config configWithLocalMember = ConfigFactory
      .parseMap(
        ImmutableMap.of(
          "wilson.local.host",
    ...
    try {
      DaggerWilsonServerComponent
        .builder()
        .wilsonDaggerModule(
          new WilsonDaggerModule(
            configWithLocalMember,
            clusterMember,
            clusterWithoutLocalMember
          )
        )
        .build()
        .create()
