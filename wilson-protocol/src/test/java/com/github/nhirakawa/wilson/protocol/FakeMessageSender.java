...
public class FakeMessageSender extends AbstractExecutionThreadService implements MessageSender {
	...
	@Override
	public CompletableFuture<VoteResponse> handleVote(ClusterMember clusterMember, VoteRequest request) {
		voteRequests.add(request);
		return CompletableFuture.completedFuture(
				VoteResponse.builder().setTerm(request.getTerm()).setVoteGranted(false).build()
		);
	}

	@Override
	public CompletableFuture<AppendEntriesResponse> handleAppendEntries(ClusterMember clusterMember, AppendEntriesRequest request) {
		appendEntriesRequests.add(request);
		return CompletableFuture.completedFuture(
				AppendEntriesResponse.builder().setTerm(request.getTerm()).setResult(AppendEntriesResult.SUCCESS).build()
		);
	}
